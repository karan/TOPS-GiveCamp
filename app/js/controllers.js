...
var emailControllers = angular.module('emailControllers', ['ngSanitize']);

emailControllers.controller('EmailListCtrl', ['$scope', '$http', '$filter', '$sce',
	function EmailListCtrl($scope, $http, $filter, $sce) {
		"use strict";
		...
		$scope.old = false;
		$scope.numResults = 200;  // if this is omitted, the select input shows a blank by default
		$scope.community = "";
		var servicesFile;
		...
		// Custom filter function for getting the UNIX timestamp from the offer for sorting
		$scope.getCommunityFromOffer = function(offer) {
			if ($scope.community == "") {
				return true;
			} else {
				return offer.community === $scope.community;
			}
		};
		...
		$scope.previewMail = function() {
			...
			$http.post('/preview_email', { "edited_json" : JSON.stringify(lastOffers) } ).success(function(data) {
				//$scope.previewText = data['txt_email'];
				$scope.previewText = data;
				$scope.showPreview = true;
				//self.previewHtml = $sce.trustAsHtml(data['html_email']); // works but messes up page styles

				//self.previewHtml = $sce.trustAsHtml("<strong>Hello</strong><br>");
				//self.
				//$scope.previewHtml = "hello";
				//self.previewHtml = "hello self";
			}).error(function(data) {
				$scope.sendErrors = data;
			});
